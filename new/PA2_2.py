...
def LucasKanade(I1, I2, features):
	I1 = np.array(I1)
	I2 = np.array(I2)
	S = np.shape(I1)
...
def LucasKanadeIterative(I1, I2, u1_, v1_):
	I1 = np.array(I1)
	I2 = np.array(I2)
	S = np.shape(I1)
	u1 = np.round(u1_)
...
def LK_Pyramid(Im1, Im2, iteration, level):
	...
	q2 = np.array(q1)
	lk_params = dict(winSize=(3, 3), maxLevel=4, criteria=(cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT, 10, 0.03))
    
    # Calculate optical flow using Lucas-Kanade method
	p1, st, err = cv2.calcOpticalFlowPyrLK(I1, I2, np.float32(q2), None, **lk_params)

	print(np.mean((p1-q2),axis=0))
	...
	(u,v) = LucasKanade(Im1, Im2,features)
	u0 = u
	v0 = v
	...
	v_l0 = v
	I_l0 = level0_I1
	u_l0[np.where(u_l0 == np.nan)] = 0
	v_l0[np.where(v_l0 == np.nan)] = 0
	print(np.mean(u_l0))
	print(np.mean(v_l0))
	#for level 1
	k = 1
	...
	v_l1 = v
	I_l1 = I1new
	u_l1[np.where(u_l1 == np.nan)] = 0
	v_l1[np.where(v_l1 == np.nan)] = 0
	print(np.mean(u_l1))
	print(np.mean(v_l1))
	...
	v_l2 = v
	I_l2 = I1new
	u_l2[np.where(u_l2 == np.nan)] = 0
	v_l2[np.where(v_l2 == np.nan)] = 0
	print(np.mean(u_l2))
	print(np.mean(v_l2))
	colours = "bgrcmykw"
	...
	dy = 0
	for x,y in q2:
		dx += u_l2[y,x] + UpSample(u_l1)[y,x] + UpSample(UpSample(u_l0))[y,x]
		dy += v_l2[y,x] + UpSample(v_l1)[y,x] + UpSample(UpSample(v_l0))[y,x]
		m += 1
	print(dx/m,dy/m)
...
'''	
t = 0.3
a = np.array(([[0,1,2,2],[2,2,0,2],[2,2,2,4]]),dtype=np.uint8)
b = (DownSample(a))
c = cv2.pyrDown(a)
print(a)
print(b)
print(UpSample(b))
print(DownSample(UpSample(b)))
print(c)
print(cv2.pyrUp(c))
