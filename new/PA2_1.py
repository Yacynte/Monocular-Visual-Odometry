...
window_size):
	...
	I2 = np.array(Image2)
	S = np.shape(I1)
	sz = np.zeros((window_size,window_size),dtype=int)
	size = sz.shape
	...
	I2_smooth = cv2.GaussianBlur(I2, size, 0)
	...
	**1/4 = 0.25** for a 2x2 filter
	...
	It = signal.convolve2d(I2_smooth,[[0.25,0.25],[0.25,0.25]],'same') + signal.convolve2d(I1_smooth,[[-0.25,-0.25],[-0.25,-0.25]],'same')
	...
	p1, st, err = cv2.calcOpticalFlowPyrLK(I1_smooth, I2_smooth, np.float32(q2), None, **lk_params)
	...
	for l in feature:
		...
		#x += j + u[i,j]
		#y += i + v[i,j]
		z.append(([j+u[i,j],i+v[i,j]]))
	q3 = np.float32(np.array(z))
...
LK_OpticalFlow(Image1, Image2,3)
